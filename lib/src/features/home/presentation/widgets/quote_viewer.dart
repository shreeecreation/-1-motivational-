...
class QuoteViewer extends StatefulWidget {
  const QuoteViewer({Key? key, required this.quote}) : super(key: key);
...
class _QuoteViewerState extends State<QuoteViewer> {
  late ScreenshotController quoteScreenshotController;

  @override
  void initState() {
    quoteScreenshotController = ScreenshotController();
    super.initState();
  }

  QuotesModel get quote => widget.quote;

  @override
  Widget build(BuildContext context) {
    final backgroundElement = context.watch<PainterSaverCubit>().state;
    return Screenshot(
      controller: quoteScreenshotController,
      child: Stack(
        children: [
          Positioned.fill(
          ...
          Positioned.fill(
            child: ColoredBox(
              color: backgroundElement.color!
                  .withOpacity(backgroundElement.isImage ? .7 : 1),
            ),
          ),
                            ...
                            children: [
                              IconButton(
                                onPressed: () {
                                  context
                                      .read<ShareCubit>()
                                      .share(quoteScreenshotController);
                                },
                                icon: const Icon(
                                  Icons.ios_share,
                                  size: 35,
